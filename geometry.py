...
gz = ceil(3/scale)   # glass z-dimension # was 8
...
sthicks = sum([ceil(i/scale) for i in thicks])
...
# carbon foam
foamx = [1,mx+2*px+1] # min and max x-values spanned by foam
foamy = [1,my+2*py+1] # min and max y-values spanned by foam
foamz = [1,2*fz+1] # min and max z-values spanned by foam

# carbon fiber
fiberx = [1,mx+2*px+1] # min and max x-values spanned by fiber
fibery = [1,my+2*py+1] # min and max y-values spanned by fiber
fiberz = [foamz[1], foamz[1]+cz]

# input materials
inx = [1+px,1+px+mx] # min and max x-values spanned by inputs
iny = [1+py,1+py+my] # min and max y-values spanned by inputs
inz = [fiberz[1],fiberz[1]+sthicks] # min z-value spanned by inputs
...
ctbz = 1+fz # x-position of center of both tubes

# module heat shunt parameters
vmod = (six[1]-six[0])*(siy[1]-siy[0])*(siz[1]-siz[0])
vshunt = (six[1]-six[0])*(hsw)*(siz[1]-siz[0])

# variables for Numba compatibility
foamz0 = foamz[0]
foamz1 = foamz[1]
fiberz0 = fiberz[0]
fiberz1 = fiberz[1]
